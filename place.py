...
"""
Dobot Magician – 色分別ロボットアーム制御スクリプト
===================================================================
このスクリプトは 15 mm 角ブロックをカラーセンサで識別し，
Dobot Magician が 2 列（列0 = 青→緑→赤，列1 = 青→赤）に自動で積み上げます。

* 干渉回避のため **MOVL(直線補間)**を使用しています。
* パラメータは `CONFIG` に一元化しているので適宜調整をしてください。
* DobotDLLはロボット本体に含まれているようなのでコメントアウトのまま実行してください。コーディング時に視認性を良くするため記述しています。
...
CONFIG = {
    ...
    'cp_vel_pct':  100,  # MOVL 速度
    'cp_acc_pct':  100,  # MOVL 加速度
